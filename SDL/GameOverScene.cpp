...
GameOverScene::GameOverScene(SceneManager& sceneManager, ContentLoader& contentLoader, ThreadPool & threadPool)
	: Scene(sceneManager, contentLoader, threadPool)
{
	HierarchyMenu::Instance()->ClearMenu();
	HierarchyMenu::Instance()->CreateBackgroundTexture(SDL_Rect{ 0, 0, 1920, 1080 }, SDL_Rect{ 0, 0, 1920, 1080 }, contentLoader.getTexture(TextureID::GameOverBG));
...
void GameOverScene::handleEvents(InputHandler * inputHandler, float dt)
{
